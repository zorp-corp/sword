            ...
            match work {
                ...
                Work11D(mut dint) => {
                    match dint.todo {
                        ...
                        Nock11DComputeResult => {
                            ...
                            if let Ok(found) =
                                match_post_hint(stack, newt, subject, hint, res)
                            {
                                res = found;
                                stack.pop::<NockWork>();
                            } else {
                ...
                Work11S(mut sint) => {
                    match sint.todo {
                        ...
                        Nock11SDone => {
                            let _ = match_post_hinted(stack, subject, sint.tag.as_noun(), res, &mut cache);
                            stack.pop::<NockWork>();
                        }
                    }
                }
                ...
                Left(atom) => {
                    if let Ok(direct) = atom.as_direct() {
                        match direct.data() {
                            ...
                            1 => {
                                *stack.push() = Work1(Nock1 { noun: formula_cell.tail() });
                            }
                            2 => {
